...
function getTableBody(scenarios) {
  let obj = {};
  scenarios.forEach((e) => {
    obj[e._filter_level_1] = obj[e._filter_level_1] || {
      label: e.filter_level_1,
    };
    obj[e._filter_level_1][e._filter_level_2] = obj[e._filter_level_1][
      e._filter_level_2
    ] || { label: e.filter_level_2 };
    obj[e._filter_level_1][e._filter_level_2][e._variable_name] = obj[
      e._filter_level_1
    ][e._filter_level_2][e._variable_name] || {
    ...
    obj[e._filter_level_1][e._filter_level_2][e._variable_name][e._year] = obj[
      e._filter_level_1
    ][e._filter_level_2][e._variable_name][e._year] || {
...
const ExploreByPathway = ({ filters, scenarios }) => {
  ...
  const renderBody = (table) => {
    ...
    const renderCells = (varNameRow, index, length) => {
      ...
      headerKeys
        .filter((e) => e)
        .forEach((e) => (!varNameRow[e] ? rowLength.push(true) : null));
      let rowData = headerKeys.map((e, i) => {
        ...
        if (i === 0)
          return (
            <td
              key={i}
              className="d-table-cell nzap-table-cell pl-2 pr-2 pt-2 pb-2 pathway cell-lead"
            >
        ...
        if (!varNameRow[e] || varNameRow[e].value === "NA")
          return (
            <td
              key={i}
              className="d-table-cell nzap-table-cell pl-2 pr-2 pt-2 pathway pb-2"
            >
              ---
            </td>
          );
      ...
      return varName.map((varNameKey, varNameIndex) => {
        ...
        return (
          <tr key={varNameIndex} className="nzap-table-row tween">
            {varNameKey !== "label"
              ? renderCells(varNameRow, varNameIndex, varName.length)
              : null}
          </tr>
        );
      });
        ...
        return (
          <React.Fragment key={l2Index}>
            <tr className="position-relative text-uppercase pt-1 pb-1 pl-3 mb-2">
              <td
                className="d-table-cell pb-2"
                colSpan={table.headers.length + 1}
              >
                <div className="l2-label pl-2 pt-1 pb-1 nzap-radius cell-lead">
                  {l2Row.label}
                </div>{" "}
              </td>
            </tr>
        ...
        <React.Fragment key={l1Index}>
          <tr className="position-relative text-uppercase pt-1 pb-1 pl-3 mb-2">
            <td
              ...
              colSpan={table.headers.length + 1}
            >
              <div className="l1-label pl-2 pt-1 pb-1 nzap-radius cell-lead">
                {l1Row.label}
              </div>{" "}
            </td>
          </tr>
        ...
        <table className="w-100 position-relative nzap-table">
          <thead>
            <tr className="nzap-table-header-row">
              <th className="pt-2 pb-2 pl-2 pr-2 position-sticky nzap-table-header-cell d-table-cell align-base cell-lead">
                Categories &amp; Subcategories
              </th>
              {table.headers.map((header, i) => (
                <th
                  key={i}
                  className="pt-2 pb-2 pl-2 pr-2 position-sticky nzap-table-header-cell d-table-cell align-top"
                >
                  <span className="label">{header.label}</span>{" "}
                  <span className="alt-name d-none d-md-inline-block">
                    {header.altName}
                  </span>
                </th>
              ))}
