  ...
  const renderBody = (table) => {
      ...
      return l2.map((l2Key, l2Index) => {
        ...
        return <React.Fragment key={l2Index}>
          <tr className="position-relative text-uppercase pt-1 pb-1 pl-3 mb-2">
            <td className="d-table-cell pb-2" colSpan={table.headers.length + 1}><div className="l2-label pl-2 pt-1 pb-1 nzap-radius cell-lead">{l2Row.label}</div> </td>
          </tr>
    ...
    return l1.map((l1Key, l1Index) => {
      ...
      return <React.Fragment key={l1Index} >
        <tr className="position-relative text-uppercase pt-1 pb-1 pl-3 mb-2">
          <td className="d-table-cell pb-2" colSpan={table.headers.length + 1}><div className="l1-label pl-2 pt-1 pb-1 nzap-radius cell-lead">{l1Row.label}</div> </td>
        </tr>
        ...
        <table className="w-100 position-relative nzap-table">
          <thead>
            <tr className="nzap-table-header-row">
              ...
              {table.headers.map((header, i) => <th key={i} className="pt-2 pb-2 pl-2 pr-2 position-sticky nzap-table-header-cell d-table-cell align-top"><span className="label">{header.label}</span> <span className="alt-name d-none d-md-inline-block">{header.altName}</span></th>)}
            </tr>
          </thead>
          <tbody>
