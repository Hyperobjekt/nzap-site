...
const NzapFilter = (props) => {
  ...
  const [activeUsState, setActiveUsState] = useState(props.rawData.usStates[0]);
  const [activeCategories, setActiveCategories] = useState([]);

  const [showMoreFilters, setShowMoreFilters] = useState(true);

  function assembleStatesDropdown(active) {
    return props.rawData.usStates.map((usState, i) =>
  ...
  function assembleCategoriesDropdown(active) {
    let categoryLabels = [...props.mutableCategories.map(e => e.label)];
    const i = categoryLabels.indexOf(active);
    if (i > -1) props.mutableCategories.splice(i, 1);
    return props.mutableCategories.map((category, i) =>
  ...
  function updateCategories(category) {
    ...
    let mutableList = props.mutableCategories.map(e => e.label);
    let selectedCategories = props.rawData.categories.filter(e => mutableList.indexOf(e.label) === -1)
