...
const NzapTable = (props) => {
  let allYears = filterData.years.map((e, i) => <div className="d-inline-block group-values clickable pl-3 pr-3 pt-2" key={i}>{e}</div>)
  let allScenarios = filterData.scenarios.map((e, i) => <div className="d-inline-block group-values clickable pl-3 pr-3 pt-2" key={i}>{e}</div>)
